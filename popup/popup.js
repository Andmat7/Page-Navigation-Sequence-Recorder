...
class PopUP {
          ...
          if (port.name == "popup_player") {
            port.onMessage.addListener(
              function (message) {
                ...
                if (message.action == "clearActions") this.reloadTable();
                if (message.action == "closePopUp") window.close();
              }.bind(this)
            );
          }
        }
  ...
  AddOnClicks() {
    ...
    document.getElementById("secuencia").onchange =
      this.saveSecuencia.bind(this);
      ...
      function (element, index) {
        ...
        if (back.mainController.state == "play") {
          if (back.player.counter > index) {
            ...
            if (back.player.counter == index) intext = "exec";
          }
        }
